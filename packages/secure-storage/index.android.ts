...
export class SecureStorage extends SecureStorageCommon {
  ...
  encryptedDataExists(keystoreKeyAlias: string): boolean {
    this.keystoreKeyAlias = keystoreKeyAlias;
    const preferences = android.preference.PreferenceManager.getDefaultSharedPreferences(Utils.android.getApplicationContext());
    return preferences.contains(SecureStorage.name + this.keystoreKeyAlias);
  }
  /**
   * @jsdoc method
   * @name encryptedDataExists
  ...
  deleteEncryptedData(keystoreKeyAlias: string): void {
    ...
    preferences
      .edit()
      .remove(SecureStorage.name + this.keystoreKeyAlias)
      .apply();
  }
  ...
  private createKey(keystoreKeyAlias: string): void {
    try {
      ...
      keyGenerator.init(new android.security.keystore.KeyGenParameterSpec.Builder(keystoreKeyAlias, android.security.keystore.KeyProperties.PURPOSE_ENCRYPT | android.security.keystore.KeyProperties.PURPOSE_DECRYPT).setBlockModes([android.security.keystore.KeyProperties.BLOCK_MODE_CBC]).setEncryptionPaddings([android.security.keystore.KeyProperties.ENCRYPTION_PADDING_PKCS7]).setKeySize(256).build());
  ...
  private initCipher(mode: number, keystoreKeyAlias: string): void {
    try {
      ...
      if (mode === javax.crypto.Cipher.ENCRYPT_MODE) {
        ...
        if (key != null) {
          const preferences = android.preference.PreferenceManager.getDefaultSharedPreferences(Utils.android.getApplicationContext());
          const ivString = preferences.getString(SecureStorage.name + keystoreKeyAlias + '_encryption_iv', null);
          if (ivString != null) {
  ...
  private tryEncrypt(secret: java.lang.String): void {
    try {
      ...
      preferences
        .edit()
        .putString(SecureStorage.name + this.keystoreKeyAlias, encrypted)
        .putString(SecureStorage.name + this.keystoreKeyAlias + '_encryption_iv', ivString)
        .apply();
    } catch (ex) {
      console.trace(ex);
      this.promiseReject({
  ...
  private tryDecrypt(): string {
    try {
      const preferences = android.preference.PreferenceManager.getDefaultSharedPreferences(Utils.android.getApplicationContext());
      const sb = new java.lang.StringBuffer(preferences.getString(SecureStorage.name + this.keystoreKeyAlias, ''));
